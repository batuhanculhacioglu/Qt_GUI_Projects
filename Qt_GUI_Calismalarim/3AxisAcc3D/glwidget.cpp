...
void GLWidget::initializeGL()
{
    ...
    GLfloat light_position[] = {100.0f, 100.0f, 100.0f, 1.0f}; // Işık kaynağının konumu
...
void GLWidget::paintGL()
{
    ...
    glRotatef(x + xRotCam, 1.0, 0.0, 0.0);
    glRotatef(z + zRotCam, 0.0, 0.0, -1.0);
    glRotatef(y + yRotCam, 0.0, 1.0, 0.0);

    drawSmoothCone(1.0f, 3.0f, 100, 22.0f);
    drawSmoothCylinder(1.0f,22,100);
    drawFlap(3,3);
    drawMotor(0.7f,0.5,100);
    drawFire();
...
void GLWidget::drawMotor(float radius, float height, int numSegments)
{
    glPushMatrix(); // Mevcut matris durumunu kaydet
    glRotatef(270,-1,0,0);
    glTranslatef(0.0f, 0.0f, 0.0f);
